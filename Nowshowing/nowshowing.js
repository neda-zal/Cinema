...
(function () {
  ...
  function displayDataList(data) {
    ...
    data.forEach(function (item, index) {
      htmlContent += `
        <div class="col-sm-6 col-md-4 col-lg-3" style="max-width:400px;">
          <div class="card mb-2">
            ...
            <div class="card-footer">
              <!-- "More" button -->
              <button class="btn btn-primary btn-show-movie" data-toggle="modal" data-target="#show-movie-modal" data-id="${item.id}">More info
              </button>
              <!-- favorite button -->
    ...
    data.forEach(item => {
      htmlContent +=
        ...
        <li class="list-group-item d-flex justify-content-between align-items-center" data-id="${item.id}">${item.title}
          ...
          <!-- "More" button -->
            <button class="btn btn-primary btn-show-movie mr-sm-3" data-toggle="modal" data-target="#show-movie-modal" data-id="${item.id}">More info
            </button>
          <!-- favorite button -->
            <button class="btn btn-info btn-add-favorite" data-id="${item.id}">+</button>
          </div>
  ...
  function showMovie(id) {
    ...
    axios
      ...
      .then(response => {
        ...
        <img src="${POSTER_URL}${data.image}" class="img-fluid" alt="Responsive image" style="width:350px;">`;
